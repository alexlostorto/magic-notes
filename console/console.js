...
/**
 * Returns an array with unique elements that are digits.
 *
 * @return {Array} An array with unique digit elements.
 */
...
/**
 * Updates the XP header element with a new text content.
 *
 * @param {Element} xpElement - The XP header element to be updated.
 */
function changeXPHeader() {
    const xpElement = document.querySelector(".status-bar-label-text");
...
/**
 * Retrieves the username from the DOM.
 *
 * @return {string} The username extracted from the DOM.
 */
function getUsername() {
    if (document.querySelector(".status-bar-label.status-bar-label-username") !== null && userName == null) {
        userName = document.querySelector(".status-bar-label.status-bar-label-username").textContent;
    }
}

/**
 * Checks if the given answer has a source URL.
 *
 * @param {any} answer - The answer to check for a source URL.
 * @return {boolean} Returns true if the answer has a source URL, false otherwise.
 */
 ...
 *
 * @param {string} inputString - The string to calculate the MD5 hash for.
...
/**
 * Updates the value of a specific key in the local storage item.
 *
 * @param {string} itemName - The name of the item in the local storage.
 * @param {string} key - The key to be updated.
 * @param {any} newValue - The new value to be assigned to the key.
 */
...
/**
 * Displays credits message in the console.
 *
 * @return {Promise<void>} - A promise that resolves when the credits message is displayed.
 */
...
/**
 * Generates a list of themes and appends it to the DOM.
 *
...
function showThemes() {
    ...
    if (document.querySelector(".themes-container") !== null) {
        return;
    }
...
/**
 * Fetches the dependencies from the given URL and adds them dynamically to the HTML document.
 *
 * @return {Promise} A promise that resolves when all the dependencies have been fetched and added.
 */
...
/**
 * Authorizes the user by obtaining an access token.
 *
 * @return {string} The authorized token.
 */
...
/**
 * Contact the database with the specified action, database, collection, and optional content.
 *
 * @param {string} action - The action to perform on the database (e.g., 'updateOne', 'findOne').
 * @param {string} database - The name of the database to connect to.
 * @param {string} collection - The name of the collection within the database.
 * @param {boolean|array|object} [content=false] - The content to update, find, or insert into the database.
 * @return {Promise<object>} - The response from the database.
 */
...
/**
 * Checks if a user exists in the database and performs necessary operations accordingly.
 *
...
/**
 * Gets the answer data in either text or latex form from the nodelist.
 *
...
/**
 * Retrieves the answer data from the keypad inputs, the selected cards, and the chosen choices.
 *
 * @return {Array} The answer data retrieved from the inputs and selections.
 */
function getAnswer() {
    let answerData = [];
...
/**
 * Retrieves the question from the question container in the HTML document.
 * If there are any question images, they will be appended to the question string.
 *
 * @return {undefined} This function does not return a value.
 */
function getQuestion() {
    ...
    if (questionContainer !== null) {
        ...
        if ((questionImages.length == 0) | questionImages[0]) {
...
/**
 * Displays the stored answer on the page.
 *
...
/**
 * Solves the bookwork code by extracting the bookwork code element from the DOM and parsing it.
 * The bookwork code is then used to retrieve the corresponding bookwork answer from the 'sparx-data' in local storage.
 * The bookwork answer is then displayed in the UI, either as an image or as a text span.
 * The function also retrieves the answer choices from the DOM and applies styling based on their similarity to the bookwork answer.
 *
 * @return {void} Does not return any value.
 */
function solveBookworkCode() {
            ...
            if (answerOptions !== null) {
                for (let i = 0; i < answerOptions.length; i++) {
                    ...
                    for (let a = 0; a < bookworkAnswer.length; a++) {
                        if (answerOption.includes(bookworkAnswer[a])) {
                            similarityCount++;
                        }
                    }
...
document.addEventListener("click", function (e) {
    if (e.target) {
        try {
            if ((e.target.id == "skill-delivery-submit-button" && e.target.innerText == "Submit") || (e.target.className == "button-text" && e.target.textContent == "Submit") || (e.target.className == "button-all" && e.target.textContent == "Submit") || (e.target.className == "button-icon button-icon-right" && e.target.parentElement.innerText == "Submit") || (e.target.parentElement.className == "button-icon button-icon-right" && e.target.parentElement.parentElement.innerText == "Submit") || (e.target.parentElement.parentElement.className == "button-icon button-icon-right" && e.target.parentElement.parentElement.parentElement.innerText == "Submit")) {
